...
env_file="${DOVIFUSE_ENV_FILE-${DV8_ENV_FILE:-$script_dir/.env}}"
if [[ -f "$env_file" ]]; then
...
BASE_DIR="${DOVIFUSE_BASE_DIR-${DV8_BASE_DIR:-$script_dir}}"
SCRIPT="${DOVIFUSE_SCRIPT_PATH-${DV8_SCRIPT_PATH:-$BASE_DIR/DoviFuse.sh}}"
LOG_FILE="${DOVIFUSE_TRIGGER_LOG_FILE-${DV8_TRIGGER_LOG_FILE:-$BASE_DIR/qbt_trigger.log}}"
LOG_MAX_BYTES="${DOVIFUSE_TRIGGER_LOG_MAX_BYTES-${DV8_TRIGGER_LOG_MAX_BYTES:-10485760}}"
JOB_LOG_DIR="$BASE_DIR/logs/jobs"
RUN_DIR="${DOVIFUSE_RUN_DIR-${DV8_RUN_DIR:-/tmp/dv8-qbt}}"
...
ARCHIVE_DIR="${DOVIFUSE_EL_RPU_DIR-${DV8_EL_RPU_DIR:-$DEFAULT_ARCHIVE_DIR}}"
DRY_RUN_FLAG="${DOVIFUSE_AUTORUN_DRY_RUN-${DV8_AUTORUN_DRY_RUN:-false}}"
MAX_PARALLEL_JOBS="${DOVIFUSE_MAX_PARALLEL_JOBS-${DV8_MAX_PARALLEL_JOBS:-1}}"
QUEUE_WAIT_SECONDS="${DOVIFUSE_QUEUE_WAIT_SECONDS-${DV8_QUEUE_WAIT_SECONDS:-15}}"
JOB_LOG_RETENTION_DAYS="${DOVIFUSE_JOB_LOG_RETENTION_DAYS-${DV8_JOB_LOG_RETENTION_DAYS:-30}}"
QBT_API_URL="${DOVIFUSE_QBT_API_URL-${DV8_QBT_API_URL:-http://127.0.0.1:8080}}"
QBT_REMOVE_CONVERTED="${DOVIFUSE_QBT_REMOVE_CONVERTED-${DV8_QBT_REMOVE_CONVERTED:-true}}"
MEDIA_ROOTS_CSV="${DOVIFUSE_MEDIA_ROOTS-${DV8_MEDIA_ROOTS:-/NAS/Movies:/NAS/TV Shows:/media/NAS/Movies:/media/NAS/TV Shows}}"
TELEGRAM_BOT_TOKEN="${DOVIFUSE_TELEGRAM_BOT_TOKEN-${DV8_TELEGRAM_BOT_TOKEN:-}}"
TELEGRAM_CHAT_ID="${DOVIFUSE_TELEGRAM_CHAT_ID-${DV8_TELEGRAM_CHAT_ID:-}}"
TELEGRAM_THREAD_ID="${DOVIFUSE_TELEGRAM_THREAD_ID-${DV8_TELEGRAM_THREAD_ID:-}}"
...
if [[ ! -x "$SCRIPT" ]]; then
  write_index "ERROR: DoviFuse launcher is not executable: $SCRIPT"
  exit 1
fi
...
(
  ...
  if [[ "$DRY_RUN_FLAG" == "true" ]]; then
    DOVIFUSE_EL_RPU_DIR="$ARCHIVE_DIR" DOVIFUSE_PROCESSING_LOG_FILE="$JOB_LOG" "$SCRIPT" --dry-run "$TARGET"
  else
    DOVIFUSE_EL_RPU_DIR="$ARCHIVE_DIR" DOVIFUSE_PROCESSING_LOG_FILE="$JOB_LOG" "$SCRIPT" "$TARGET"
  fi
  rc=$?
