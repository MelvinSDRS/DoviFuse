...
- `DoviFuse.sh` is now a launcher for the Rust converter (`dovifuse_converter`).
- `dovifuse_converter` performs detection, extraction, DV metadata processing (`dovi_tool`), remux, validation, and cleanup.
- `qbt_autorun_wrapper.sh` runs queued background jobs with locking, per-job logs, and optional qBittorrent cleanup.

`DoviFuse.sh` tries converters in this order:

1. `DOVIFUSE_CONVERTER_BIN` (if set)
2. `tools/dovifuse_converter`
3. `dovifuse_converter/target/release/dovifuse_converter`
4. `$CARGO_TARGET_DIR/release/dovifuse_converter` (if `CARGO_TARGET_DIR` is set)
5. Build from source with `cargo build --release --manifest-path dovifuse_converter/Cargo.toml`

## Repository Layout

- `DoviFuse.sh`: launcher and fallback build logic for `dovifuse_converter`.
- `dovifuse_converter/`: Rust conversion engine.
- `qbt_autorun_wrapper.sh`: queue/lock wrapper for torrent-triggered runs.
- `config/dovifuse.json`: `dovi_tool` editor config used during conversion.
- `tools/`: optional bundled binaries (`mkvextract`, `mkvmerge`, `mediainfo`, `dovi_tool`, `dovifuse_converter`).
...
- Linux with `bash`.
- `cargo` (required if no ready-to-run `dovifuse_converter` binary is available).
...
Both `DoviFuse.sh` and `qbt_autorun_wrapper.sh` auto-load `.env` from repo root.
You can point to a different env file with `DOVIFUSE_ENV_FILE=/path/to/file.env`.
...
./DoviFuse.sh --dry-run /path/to/movie.mkv
...
- `--archive-dir <path>`: explicit standard-mode EL+RPU archive directory,
  overriding `DOVIFUSE_EL_RPU_DIR`. Cannot be combined with `-n` or another mode.
...
- `--tmp-dir <path>`: put large intermediate streams in a separate scratch
  directory (or set `DOVIFUSE_TMP_DIR`). Each run receives its own subdirectory.
...
```bash
./DoviFuse.sh --check /path/to/movie.dv8.mkv
```
...
./DoviFuse.sh --hybrid --dry-run /path/to/dv_source.mkv /path/to/hdr_target.mkv

# Convert with default output naming (<target>.DV8.Hybrid.mkv)
./DoviFuse.sh --hybrid /path/to/dv_source.mkv /path/to/hdr_target.mkv

# Convert with explicit output path
./DoviFuse.sh --hybrid -o /path/to/output.mkv /path/to/dv_source.mkv /path/to/hdr_target.mkv
```
...
- Queues jobs with per-target and slot locks (`DOVIFUSE_MAX_PARALLEL_JOBS`).
- Writes index log to `qbt_trigger.log` and detailed logs to `logs/jobs/*.log`.
- Treats `Not a DV7 file` as a non-fatal outcome.
- Captures and repoints media hardlinks for converted files (`DOVIFUSE_MEDIA_ROOTS`).
- Optionally stops/removes converted torrents in qBittorrent while keeping files (`DOVIFUSE_QBT_REMOVE_CONVERTED=true`).
...
- `DOVIFUSE_ENV_FILE`: optional path to env file (default `<repo>/.env`).
- `DOVIFUSE_BASE_DIR`: repo base directory (default wrapper script directory).
- `DOVIFUSE_SCRIPT_PATH`: converter launcher path (default `$DOVIFUSE_BASE_DIR/DoviFuse.sh`).
- `DOVIFUSE_CONVERTER_BIN`: force converter binary path.
- `DOVIFUSE_EL_RPU_DIR`: archive directory override (default `/NAS/EL_RPU/` or `/media/NAS/EL_RPU/`).
- `DOVIFUSE_PROCESSING_LOG_FILE`: conversion log path override.
- `CARGO_TARGET_DIR`: optional target directory used by launcher fallback probing.

### Wrapper

- `DOVIFUSE_AUTORUN_DRY_RUN` (default `false`)
- `DOVIFUSE_MAX_PARALLEL_JOBS` (default `1`)
- `DOVIFUSE_QUEUE_WAIT_SECONDS` (default `15`)
- `DOVIFUSE_JOB_LOG_RETENTION_DAYS` (default `30`)
- `DOVIFUSE_TRIGGER_LOG_FILE` (default `$DOVIFUSE_BASE_DIR/qbt_trigger.log`)
- `DOVIFUSE_TRIGGER_LOG_MAX_BYTES` (default `10485760`)
- `DOVIFUSE_RUN_DIR` (default `/tmp/dovifuse-qbt`)
- `DOVIFUSE_QBT_API_URL` (default `http://127.0.0.1:8080`)
- `DOVIFUSE_QBT_REMOVE_CONVERTED` (default `true`)
- `DOVIFUSE_MEDIA_ROOTS` (default `/NAS/Movies:/NAS/TV Shows:/media/NAS/Movies:/media/NAS/TV Shows`)
- `DOVIFUSE_EL_RPU_DIR` (passed through to converter)
...
`macapp/` contains the Apple Silicon SwiftUI **DoviFuse** application and its
...
The output is `dist/DoviFuse.app`. The app bundles the converter, dovi_tool,
...
```bash
# Build converter
cargo build --release --manifest-path dovifuse_converter/Cargo.toml
...
```bash
./DoviFuse.sh --check --report /tmp/movie-check.json /path/to/movie.mkv
```
...
`~/Library/Application Support/DoviFuse/Reports/`. **Show report** reveals it in
