...
class ReferenceGateTests(unittest.TestCase):
    ...
    def test_changed_preservation_evidence_is_rejected(self):
        ...
        evidence = self.root/'tests/fixtures/preservation-evidence.json'
        evidence.write_text('{}')
        gate.update(status='passed', evidence=[{
            'path': 'tests/fixtures/preservation-evidence.json',
            'sha256': sha256(evidence),
            'reviewer': 'Regression fixture',
            'method': 'Synthetic acceptance record used only by this test',
        }])
        (self.root/'tests/fixtures/release-gates.json').write_text(json.dumps(gates))
        self.verify()
